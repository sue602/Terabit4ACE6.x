...
BaseConnector::open (TRB_Proactor *proactor)
{
                                    ...
                                    this->proactor ()) == -1)
    {
        ACE_ERROR_RETURN ((LM_ERROR,
            ACE_TEXT ("(%t) BaseConnector: TRB_Asynch_Connect::open failed")),
            -1);
    }
                        ...
                        const void *act)
{
                                       ...
                                       act) == -1)
    {
        ACE_ERROR_RETURN ((LM_ERROR,
                       ACE_TEXT ("%N:%l %p\n"),
                       ACE_TEXT ("BaseConnector::connect")),
                      -1);
    }
...
BaseConnector::handle_connect (const TRB_Asynch_Connect::Result &result)
{
    ...
    if (error != 0 || handle == ACE_INVALID_HANDLE)
    {
        ...
        ACE_DEBUG ((LM_DEBUG,
                    ...
                    reinterpret_cast<long> (result.act ()),
                    error,
    ...
    else 
    {
        ...
        {
            ACE_ERROR ((LM_ERROR,
                ACE_TEXT ("(%t) BaseConnector: %p\n"),
                ACE_TEXT ("Set blocking mode failed")));
        }
    }

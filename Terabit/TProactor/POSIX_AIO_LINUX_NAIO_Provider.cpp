...
TRB_POSIX_AIO_LINUX_NAIO_Provider::Notification_File::open ()
{
    ...
    if (this->handle_ == ACE_INVALID_HANDLE)
    {
        ACE_ERROR_RETURN ((LM_ERROR,
            ACE_TEXT ("(%t) ACE_POSIX_AIO_LINUX_Provider:: %p\n"),
            ACE_TEXT ("<Notification_File::open> failed")),
            -1);
    }
...
TRB_POSIX_AIO_LINUX_NAIO_Provider::init ()
{
    ...
    if (rc != 0)
    {
        this->context_ = 0;
        ACE_ERROR_RETURN ((LM_ERROR,
            ACE_TEXT ("(%t) ACE_POSIX_AIO_LINUX_Provider::init %p\n"),
            ACE_TEXT ("<io_setup> failed")),
            -1);
    }
...

    {
        ACE_ERROR_RETURN ((LM_ERROR,
            ACE_TEXT ("(%t) ACE_POSIX_AIO_LINUX_Provider::init %p\n"),
            ACE_TEXT ("<register_handle> failed")),
            -1);
    }
    return 0;
}
...
TRB_POSIX_AIO_LINUX_NAIO_Provider::wait_for_events (ACE_Time_Value *wait_time)
{
    ...
    if (this->num_remain_ < 0)
    {
        this->num_remain_ = 0;
        ACE_ERROR_RETURN ((LM_ERROR,
            ACE_TEXT ("(%t) io_getevents error=%d %p\n"),

                                        ...
                                        const void *act)
{
  ...
  if (this->asynch_connect_.connect (ACE_INVALID_HANDLE,
    ...
    ACE_ERROR_RETURN ((LM_ERROR,
                       ACE_TEXT ("%N:%l %p\n"),
                       ACE_TEXT ("TRB_Asynch_Connector::connect")),
                      -1);
  return 0;
}
...
TRB_Asynch_Connector<HANDLER>::handle_connect (const TRB_Asynch_Connect::Result &result)
{
        ...
        ACE::clr_flags (result.connect_handle (), ACE_NONBLOCK) != 0)
    {
        ...
        ACE_ERROR ((LM_ERROR,
            ACE_TEXT ("%p\n"),
            ACE_TEXT ("TRB_Asynch_Connector::handle_connect : Set blocking mode")));
    }
    ...
    if (!error)
    {
        ...
        if (new_handler == 0)
        {
            error = 1;
            ACE_ERROR ((LM_ERROR,
                ACE_TEXT ("TRB_Asynch_Connector::handle_connect : make_handler failed")));
        }
    }

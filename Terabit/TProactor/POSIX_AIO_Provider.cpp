    ...
    if (rc < 0)
    {
        if (translate_errno() != EWOULDBLOCK)
        {
            ACE_ERROR_RETURN ((LM_ERROR,
                ACE_TEXT("(%P|%t) TRB_POSIX_AIO_Provider::Notification_Pipe::%p\n"),
                ACE_TEXT("notify failed")),
                -1);
        }
        return 0;
    }
...
TRB_POSIX_AIO_Provider::Notification_Pipe::read ()
{
    ...
    {
        ACE_ERROR_RETURN ((LM_ERROR,
            ACE_TEXT("(%P|%t) TRB_POSIX_AIO_rovider::Notification_Pipe::%p\n"),
            ACE_TEXT("read failed")),
            -1);
    }

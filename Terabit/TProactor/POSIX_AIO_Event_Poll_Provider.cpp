    ...
    if (rc < 0)
    {
        ...
        default:
            ACE_ERROR_RETURN ((LM_ERROR,
                ACE_TEXT("(%t) TRB_POSIX_AIO_Event_Poll_Strategy::declare_interest : %p\n"),
                ACE_TEXT("epoll_ctl failed")),
                -1);
        }
    }
    return 0;
...
TRB_POSIX_AIO_Event_Poll_Provider::clear_interest_i (int fd)
{
    ...
    if (rc < 0)
    {
        ...
        default:
          ACE_ERROR_RETURN ((LM_ERROR,
            ACE_TEXT("(%t) TRB_POSIX_AIO_Event_Poll_Strategy::clear_interest : %p\n"),
            ACE_TEXT("epoll_ctl failed")),
            -1);
        }
    }
...
TRB_POSIX_AIO_Event_Poll_Provider::wait_for_ready_fd (ACE_Time_Value * wait_time)
{
    ...
    if (wait_time != 0 )
    {
        timeout =  static_cast<long> (wait_time->msec ());
    }

                        ...
                        const void *act)
{
                                     ...
                                     act) == -1)
    {
        ACE_ERROR_RETURN ((LM_ERROR,
                       ACE_TEXT ("%N:%l %p\n"),
                       ACE_TEXT ("TRB_Asynch_Connector2::connect")),
                      -1);
    }
                ...
                const TRB_Asynch_Connect::Result &result)
{
       ...
       ACE::clr_flags (result.connect_handle (), ACE_NONBLOCK) != 0)
    {
       ...
       ACE_ERROR ((LM_ERROR,
          ACE_TEXT ("%p\n"),
          ACE_TEXT ("TRB_Asynch_Connector2::handle_connect: set blocking mode")));
    }
    ...
    if (!error)
    {
        ...
        if (new_handler == 0)
        {
          error = 1;
          ACE_ERROR ((LM_ERROR,
             ACE_TEXT ("TRB_Asynch_Connector2::handle_connect: make_handler failed")));
        }
    }

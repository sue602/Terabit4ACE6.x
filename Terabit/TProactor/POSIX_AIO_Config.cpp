...
TRB_POSIX_AIO_Config::dump (void) const
{
    ...
    ACE_DEBUG ((LM_DEBUG, 
        ACE_TEXT ("Max. number aios : %u\n"),
        (unsigned int)this->max_num_aio_));

    switch (this->processor_type())
    {
    case PCT_NONE:      str_tmp = ACE_TEXT("DUMMY (not exist)"); break;
    case PCT_DEDICATED: str_tmp = ACE_TEXT("DEDICATED"); break;
    case PCT_SHARED:    str_tmp = ACE_TEXT("SHARED"); break;
    default:            str_tmp = ACE_TEXT("??????"); break;
    }

    ACE_DEBUG ((LM_DEBUG, 
        ACE_TEXT ("AIO Processor type  : %X %s\n"), 
        this->processor_type_,
        str_tmp));

    switch (this->provider_type ())
    {
    case PVT_NONE       :   str_tmp = ACE_TEXT("DUMMY (not exist)"); break;
    case PVT_POSIX_AIOCB:   str_tmp = ACE_TEXT("POSIX_AIOCB"); break;
    case PVT_POSIX_SIG  :   str_tmp = ACE_TEXT("POSIX_SIG  "); break;
    case PVT_POSIX_CB   :   str_tmp = ACE_TEXT("POSIX_CB   "); break;
    case PVT_SUN_AIO    :   str_tmp = ACE_TEXT("SUN_AIO    "); break;
    case PVT_LINUX_NAIO :   str_tmp = ACE_TEXT("LINUX_NAIO "); break;
    case PVT_SELECT     :   str_tmp = ACE_TEXT("SELECT     "); break;
    case PVT_POLL       :   str_tmp = ACE_TEXT("POLL       "); break;
    case PVT_EPOLL      :   str_tmp = ACE_TEXT("EPOLL      "); break;
    case PVT_DEVPOLL    :   str_tmp = ACE_TEXT("DEVPOLL    "); break;
    case PVT_SUNPORT    :   str_tmp = ACE_TEXT("SUNPORT    "); break;
    case PVT_KQUEUE     :   str_tmp = ACE_TEXT("KQUEUE     "); break;
    default             :   str_tmp = ACE_TEXT("??????");      break;
    }
    ACE_DEBUG ((LM_DEBUG, 
        ACE_TEXT ("AIO Provider type   : %X %s\n"), 
        this->provider_type_,
        str_tmp));
...
{ 
    ...
    if ((int) max_num_files <= 0)
        max_num_files = static_cast<size_t>(ACE::max_handles ());
